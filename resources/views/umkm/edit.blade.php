...
<body>
@include('partials.navbar', ['title' => 'Edit Data UMKM'])
<div class="container">
    <div class="card">
        @if($errors->any()) <div class="error">{{ $errors->first() }}</div> @endif
        <form method="POST" action="/umkm/{{ $umkm->id }}/update">
            @csrf
            @if(auth()->user()->role === 'admin')
                <label>Akun UMKM</label>
                <select name="user_id" required>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ $umkm->user_id == $user->id ? 'selected' : '' }}>{{ $user->name }} - {{ $user->email }}</option>
                    @endforeach
                </select>
            @endif
            ...
            <label>Alamat</label>
            <textarea name="alamat" rows="3" required>{{ old('alamat', $umkm->alamat) }}</textarea>
            <label>Kategori Usaha</label>
            <input type="text" name="kategori_usaha" value="{{ old('kategori_usaha', $umkm->kategori_usaha) }}">
            <label>Pin Lokasi UMKM</label>
            <div id="map"></div>
            <div class="map-note">Klik lokasi UMKM di peta atau geser marker untuk memperbarui titik alamat.</div>
            <div class="coords">
                <input type="text" name="latitude" id="latitude" value="{{ old('latitude', $umkm->latitude) }}" placeholder="Latitude" readonly>
                <input type="text" name="longitude" id="longitude" value="{{ old('longitude', $umkm->longitude) }}" placeholder="Longitude" readonly>
            </div>
